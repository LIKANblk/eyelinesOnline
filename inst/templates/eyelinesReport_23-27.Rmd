...
TN_cell <- length(which(events$quick_fixation == T & events$activation == F 
                        & events$field_type == 'field'  & events$changed_selection == F))
FP_ball <- length(which((events$false_alarm == T | events$changed_selection == T)& events$field_type == 'ball'))
FP_cell <- length(which((events$false_alarm == T | events$changed_selection == T) & events$field_type == 'field'))
...
for (i in 1:length(experiment)){
  if(experiment[[i]]$file_data$record_type == 'test' || experiment[[i]]$file_data$record_type == 'random'){
    cat(
      ...
      ') - ', sum(experiment[[i]]$events$changed_selection == TRUE), ' changed selection (without report FA) and '
      , sum(experiment[[i]]$events$false_alarm), ' FA reported \n', sep = "")
  }
}
```
