...
for (i in 1:length(experiment)){
  if(experiment[[i]]$file_data$record_type == 'test' || experiment[[i]]$file_data$record_type == 'random'){
    if(!is.null(experiment[[i]]$file_data$changed_selection)) {
      changed_selection <- experiment[[i]]$file_data$changed_selection
    } else {
      changed_selection <- 0
    }
    cat(
      ...
      ') - ', changed_selection, ' changed selection and '
