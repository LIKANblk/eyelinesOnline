draw_random_epochs <- function(experiment, clf_response=c('true_positive', 'true_negative', 'false_negative')) {
  ...
  for(exp in experiment){
    if(exp$file_data$record_type == 'random'){
  ...
  less_than_600_title <- paste0('Random classifier', clf_response, '. ', 'Dwell < 600', '\n',
                                ...
                                " cell epochs")
  if(clf_response != 'false_negative'){ 
    more_than_600_title <- paste0('Random classifier', clf_response, '. ', 'Dwell >= 600', '\n',
                                  sum(summary_table$quick_fixation == qf & 
                                        summary_table$activation == act &
                                        summary_table$dwell_time >= 600 &
                                        summary_table$field_type == 'ball'),
    ...
    more_than_600_title <- paste0('Random classifier', clf_response, '. ', 'Dwell == 1000', '\n',
                                  sum(summary_table$quick_fixation == qf & 
                                        summary_table$activation == act &
                                        summary_table$dwell_time >= 600 &
                                        summary_table$field_type == 'ball'),
  ...
  if(clf_response != 'false_negative'){
    if(exists("p1") && exists("p2")){
      ...
      multiplot(pl_blank, pl_blank, cols=2)
    }
  } else {
