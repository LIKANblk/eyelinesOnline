prepare.data <- function(signal, actions, epoch_size, sRate, left_border)
{
  ...
  eegT <- array(dim = c(fixationDuration_t, nChannels-1, length(eventsT_t)))
  
  for (i in 1:length(eventsT_t))
  {
    eegT[ , , i] <- signal[(eventsT_t[i]+left_border+1):(eventsT_t[i]+fixationDuration_t+left_border), -ncol(signal)]
  }
  
  eegNT = array(dim = c(fixationDuration_t, nChannels-1, length(eventsNT_t)))
  for (i in 1:length(eventsNT_t))
  {
    eegNT[ , , i] <- signal[(eventsNT_t[i]+left_border+1):(eventsNT_t[i]+fixationDuration_t+left_border), -ncol(signal)]
